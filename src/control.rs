...
impl Connection {
    /// Spawn `tmux -C attach -t <session>` against the current server
    /// (taken from `$TMUX` in the spawned process's environment), binding
    /// the control client to exactly one session: when that session is
    /// killed the client gets `%exit`, which is the handler's shutdown
    /// signal. Drains the initial `%begin/%end` frame so callers see a
    /// clean channel.
    ///
    /// The handler must never send `refresh-client -C` (a size) on this
    /// connection — a control client without a size does not participate in
    /// session sizing, so it can't resize the user's real client.
    pub fn attach(session: &str) -> Result<Self, String> {
    ...
    // The tmux child is intentionally not waited on: its lifetime is bound
    // to our pipe FDs, and closing them detaches the control client.
